...
public class TestService {
 ...
 public void ajouterTest(Test p){
        String query ="insert into test(duree,nbrp,titre,sujet_id) values('"+p.getDuree()+"','"+p.getNbrp()+"','"+p.getTitre()+"','"+p.getSujet_id()+"')";
        ...
        try {
            ...
            while(rs.next()){
               Test p = new Test();
                ...
                p.setSujet_id(rs.getInt("sujet_id"));
                list.add(p);
            }
        } catch (SQLException ex) {
            System.out.println(ex.getMessage());
...
//
 ...
 public void supprimerTest(String titre) {
    ...
    public List<Test> afficher1() {
        ...
        try {
            String req = "SELECT duree,nbrp,titre,sujet_id from test";
...
String sq13="UPDATE `test`SET `duree`=?,`nbrp`=?,`titre`=?,`sujet_id`=? WHERE duree =?";
...
public List<Test> ListClasse() {
      ...
      ResultSet e = pst.executeQuery();
            ...
            pre.setTitre(e.getString("titre"));
             pre.setSujet_id(e.getInt("sujet_id"));
